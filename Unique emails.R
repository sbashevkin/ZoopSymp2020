...
New_emails3<-read_sheet("https://docs.google.com/spreadsheets/d/1uqkViNAyUZ6eEYjXZt05nbD_0ge5xhGainiV0IUiLO4", sheet=1)%>%
  mutate(Datetime=force_tz(Timestamp, "America/Los_Angeles"))%>%
  filter(Datetime>as.POSIXct("2020-10-26 15:00"))%>%
  select(Email)%>%
  distinct()

write_csv(New_emails3, "~/New Zoop emails3.csv")